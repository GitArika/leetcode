...
- At time 1, person 1 shares the secret with person 2, and person 2 shares the secret with person 3. Note that person 2 can share the secret at the same time as receiving it.
...
class Solution:
  def findAllPeople(self, n: int, meetings: List[List[int]], firstPerson: int) -> List[int]:
    ...
    for [x, y, time] in meetings:
      current_meet_group = set([x,y])
      merged = False
      print(current_meet_group)
      if not meeting_map.get(time):
        meeting_map[time] = [set()]
